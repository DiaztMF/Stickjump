...
class GameScene extends Phaser.Scene {
    ...
    create() {
        ...
        this.levelText = this.add.text(16, 16, "LEVEL 1", {
            fontSize: "32px", fill: "#fff", fontStyle: "bold", stroke: "#000", strokeThickness: 2,
        });
        this.add.text(16, 56, "ARROWS to Move  |  SPACE to Jump", {
            fontSize: "18px", fill: "#fff", stroke: "#000", strokeThickness: 4,
        });
        ...
        this.levels = {
            1: {
                ...
                spikes: [
                    { x: 250, y: 640 }, { x: 280, y: 640 }, { x: 450, y: 540 },
                ],
                finish: { x: 700, y: 400 },
            },
            2: {
                ...
                spikes: [
                    { x: 200, y: 540 }, { x: 350, y: 470 }, { x: 380, y: 470 },
                    { x: 550, y: 390 }, { x: 580, y: 390 },
                ],
                movingPlatforms: [
            ...
            3: {
                ...
                spikes: [
                    { x: 180, y: 540 }, { x: 210, y: 540 }, { x: 330, y: 470 },
                    { x: 500, y: 410 }, { x: 530, y: 410 }, { x: 670, y: 340 },
                ],
                movingPlatforms: [
        ...
        this.physics.add.overlap(this.player, this.finishSprite, this.reachFinish, null, this); // Note: finishSprite recreated per level
    ...
    update() {
        ...
        this.movingPlatformsArray.forEach((plat) => {
            ...
            if (plat.body.touching.up && this.player.body.touching.down) {
                ...
                if (Math.abs(this.player.y - (plat.y - plat.height/2 - this.player.height/2)) < 5) {
    ...
    updateStickman() {
        ...
        const armOffset = Math.sin(this.time.now / 100) * (this.player.body.velocity.x !== 0 ? 5 : 0);
        this.stickmanGraphics.beginPath();
        ...
        const legOffset = Math.sin(this.time.now / 50) * (this.player.body.velocity.x !== 0 ? 8 : 0);
        this.stickmanGraphics.beginPath();
    ...
    createFlag(x, y) {
        ...
        this.flagGraphics.fillTriangle(x + 2, y - 45, x + 2, y - 25, x + 27, y - 35);
        this.flagGraphics.lineStyle(2, 0x008800, 1);
        this.flagGraphics.strokeTriangle(x + 2, y - 45, x + 2, y - 25, x + 27, y - 35);
    }
    ...
    reachFinish(player, finish) {
        ...
        if (this.currentLevel > 3) {
            ...
            this.levelText.setPosition(250, 250);
            this.player.body.setEnable(false); // Stop moving
            ...
            this.createLevel(this.currentLevel);
            this.resetPlayer();
            // Small delay to prevent instant re-trigger
            this.time.delayedCall(100, () => { this.isChangingLevel = false; });
        }
    }
}
